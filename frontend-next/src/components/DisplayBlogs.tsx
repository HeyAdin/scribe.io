...
export function BlogsCard({ authorName, title, content, publishedDate, id, likeCount }: BlogsCardInput) {
  ...
  return <div className="border-t w-full mt-4 p-4 flex flex-col gap-4" onClick={() => { router.push(`/blogs/${id}`) }}>
    ...
    <div className="flex gap-4 items-center w-full ">
      <p className="font-bold text-gray-600 cursor-pointer relative group">Read full blog
        <span className="absolute left-0 -bottom-1 h-[2px] w-0 bg-black transition-all duration-300 group-hover:w-full"></span>
      </p>
      <p className="text-gray-500">{`${Math.ceil(content.length / 200)} minute read`}</p>
      <div className="flex gap-2 text-neutral-600 font-black cursor-pointer "><img className="" src={'/images/like.png'} height={15} width={25} />{likeCount} likes</div>
    </div>
  </div>
}
